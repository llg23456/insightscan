...
from src.utils import get_setting, init_db, setup_logging
...
def run_defense_mode(
    ...
    for i in range(rounds):
        ...
        scan_detect = ScanBehaviorDetector(lookback_minutes=lookback_minutes).detect()
    ...
    report_md = _render_defense_report(
        scan_detect, promisc, local_scan, rules_result, unique_events
    )
...
def _render_defense_report(
    ...
    lines = [
        ...
        "## 1. 被扫描检测",
        "",
        f"- 检测时间窗口: 最近 {scan_detect.get('lookback_minutes', 60)} 分钟",
        f"- 检测到事件数: {scan_detect.get('total_events', 0)}",
        f"- 明确扫描行为(Nmap/SYN等): {summary.get('scan_events', 0)} 次",
    ...
    if events:
        ...
        for e in events[:15]:
            lines.append(
                ...
                f"{e.get('attack_type')} [{e.get('severity')}]"
            )
        lines.append("")
